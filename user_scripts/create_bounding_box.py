...
relations.create_formula(
    "Formula.Left", "", length_left, parameter_bbox_left.name.split("\\")[1]
)
relations.create_formula(
    "Formula.Right", "", length_right, parameter_bbox_right.name.split("\\")[1]
)
...
plane_origin = hybrid_shape_factory.add_new_plane2_lines(
    ref_origin_line_1, ref_origin_line_2
)
ref_plane_origin = part.create_reference_from_object(plane_origin)
plane_origin = hybrid_shape_factory.add_new_plane_offset_pt(
    ref_plane_origin, ref_point_6
)
ref_plane_origin = part.create_reference_from_object(plane_origin)
gs_bbox.append_hybrid_shape(plane_origin)

plane_offset = hybrid_shape_factory.add_new_plane_offset_pt(
    ref_plane_origin, ref_point_5
)
