...
for f in files:
    with CATIADocHandler(file_name=f) as handler:
        ...
        specs_window.layout = cat_specs_and_geom_window_layout.index(
            "catWindowGeomOnly"
        )
        ...
        if img_save_name.is_file():
            logger.warning(
                f'Image file: "{img_save_name}" already exists! Image will be replaced.'
            )
            existing_files_warning.append(img_save_name)
        viewer_3d.capture_to_file(
            cat_capture_format.index("catCaptureFormatBMP"), str(img_save_name)
        )
...
duplicates = [
    item for item, count in collections.Counter(img_files).items() if count > 1
]
if len(duplicates) > 0:
    logger.warning(
        f"There are duplicate image file names: {duplicates}. Duplicates will be removed."
    )
