...
from pycatia.knowledge_interfaces.knowledge_activate_object import (
...
class Relation(KnowledgeActivateObject):
    ...
    def context(self) -> AnyObject:
        ...
        return AnyObject.new(self.relation.Context)
    ...
    @property
    def in_parameters(self) -> list[str]:
        return [self.get_in_parameter(i) for i in range(1, self.nb_in_parameters + 1)]

    @property
    def out_parameters(self) -> list["Parameter"]:
        return [self.get_out_parameter(i) for i in range(1, self.nb_out_parameters + 1)]
    ...
    def get_in_parameter(self, i_index: int) -> AnyObject:
        ...
        :rtype: AnyObject
        """
        return AnyObject.new(self.relation.GetInParameter(i_index))
    ...
    def __repr__(self):
        return f'Relation(name="{self.name}", comment="{self.comment}")'
    ...
    def to_dict(self):
        return {
            ...
            "value": self.value,
            "in_parameters": [x.name for x in self.in_parameters],
            "out_parameters": [x.to_dict() for x in self.out_parameters],
        }
