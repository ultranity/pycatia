...
class DrawingPictures(Collection[DrawingPicture]):
    """
    .. note::
        :class: toggle
    ...
    def add(
        self, i_drawing_picture_path: Path, i_position_x: float, i_position_y: float
    ) -> DrawingPicture:
        """
        ...
        if not absolute_path.is_file():
            raise CATIAApplicationException(
                f'Could not find image file: "{absolute_path}".'
            )
        return DrawingPicture(
            self.drawing_pictures.Add(absolute_path, i_position_x, i_position_y)
        )
