...
def convert_units(number: str, unit: str) -> float:
    ...
    try:
        ...
        raise FloatingPointError(
            "Input {number} can not be converted to float(). Check csv data."
        )
...
def csv_reader(
    file_name: str, units: str, delimiter: str = ","
...
def create_points(
    part: Part, file_name: str, units: str = "mm", geometry_set_name: str = "New_Points"
) -> None:
    """
    ...
    for point in points:
        ...
        print(
            f"Added point: {point['name']}. Time taken = {round(time_taken, 3)} seconds",
            end="\r",
        )
